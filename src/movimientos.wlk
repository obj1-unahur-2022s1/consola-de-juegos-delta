...
class Direccion {
	...
	method proximaPosicion(posicion) {
		...
		if (self.esIgual(derecha) && self.esBordeDerecho(posicion)) {
			siguientePosicion = game.at(posicion.x(), posicion.y())
	...
	// Apareciendo del otro lado, estilo pacman
	
	method proximaPosicionEstiloPacman(posicion) {
		var siguientePosicion = self.siguiente(posicion)
		if (self.esIgual(derecha) && self.esBordeDerecho(posicion)) {
